...
#include "confusion_example/SensorEnumDefinition.h"
...
std::vector<double> saveStaticParameterValues(
    const StaticParameterVector& staticParameters) {
...
void runConFusor(std::deque<std::shared_ptr<ImuMeas>>& imuMeasVec,
  ...
  const bool dropEverySecondState =
      propertyTree.get<bool>("vimhe.dropEverySecondState");
  ...
  // fly
  auto firstState =
      std::dynamic_pointer_cast<ImuState>(conFusor.stateVector_.front());
  firstState->setTagReferenceFrameOffsets(&referenceFrameOffsets);

  conFusor.stateVector_.front()->setInitialStateWeighting(
  ...
  if (optimizeTci) {
    ...
    confusion::Parameter qciParam(T_c_i.rot.coeffs().data(), 4, "q_c_i", false,
    ...
    conFusor.addStaticParameter(
        confusion::Parameter(T_c_i.trans.data(), 3, "t_c_i", true));
    conFusor.addStaticParameter(
        confusion::Parameter(T_c_i.rot.coeffs().data(), 4, "q_c_i", true,
                             std::make_shared<QuatParam>()));
  }
  ...
  gravityRotInitialWeighting /=
      solverParameters.tagMeasCalibration_.ftr_init_stddev_;
  ...
  // Step through the pose measurements, adding imu measurements and solving
  // along the way
  size_t poseIndex = 0;
  ...
  for (int i = 0; i < cycleTimeIndex; ++i) {
    ...
    if (cycleTimes[i] > maxCycleTime) maxCycleTime = cycleTimes[i];
  }
  avgCycleTime /= (double)cycleTimeIndex;
  std::cout << "\n\nConFusor avg cycle time: " << avgCycleTime
            << "; max cycle time: " << maxCycleTime << std::endl;
  ...
  conFusor.staticParameters_.getParameter(T_c_i.rot.coeffs().data())
      ->unsetConstant();

  std::vector<double> conFusorStaticParameters =
      saveStaticParameterValues(conFusor.staticParameters_);
  ...
  std::vector<double> batchFusorStaticParameters =
      saveStaticParameterValues(batchFusor.staticParameters_);
  ...
  for (std::size_t i = 0; i < conFusorStaticParameters.size(); ++i) {
    EXPECT_NEAR(conFusorStaticParameters[i], batchFusorStaticParameters[i],
                0.1);
  }
...
TEST(ConFusorTest, ShouldPass) {
  ...
  std::string confusion_pkg_path = ros::package::getPath("confusion_ros");

  std::string vimheConfig = confusion_pkg_path + "/test/include_test/vimhe_config.xml";

  boost::property_tree::read_info(vimheConfig, propertyTree);

  solverParameters.twi_init_stddev =
      propertyTree.get<double>("vimhe.twi_init_stddev");
  solverParameters.qwi_init_stddev =
      propertyTree.get<double>("vimhe.qwi_init_stddev");
  solverParameters.vwi_init_stddev =
      propertyTree.get<double>("vimhe.vwi_init_stddev");
  solverParameters.ba_init_stddev =
      propertyTree.get<double>("vimhe.ba_init_stddev");
  solverParameters.bg_init_stddev =
      propertyTree.get<double>("vimhe.bg_init_stddev");
  aprilTagParameters.tagMeasCalibration_.ftr_init_stddev_ =
      propertyTree.get<double>("vimhe.ftr_init_stddev");
  aprilTagParameters.t_c_i_init_stddev =
      propertyTree.get<double>("vimhe.t_c_i_init_stddev");
  aprilTagParameters.q_c_i_init_stddev =
      propertyTree.get<double>("vimhe.q_c_i_init_stddev");
  aprilTagParameters.tagMeasCalibration_.tagSize_ =
      propertyTree.get<double>("vimhe.tagSize");
  solverParameters.imuCalibration_.gravityMagnitude_ =
      propertyTree.get<double>("vimhe.gravityMagnitude");
  ...
  aprilTagParameters.tag_corner_stddev_ =
      propertyTree.get<double>("vimhe.tag_corner_stddev");
  ...
  // MHE solution
  const std::string bag_in = confusion_pkg_path + "/test/include_test/vi_test.bag";
  std::deque<std::shared_ptr<ImuMeas>> imuMeasVec;
  std::vector<std::shared_ptr<TagMeas>> tagMeasVec;

  //	initializeProjMat();
  importDataFromBag(bag_in, imuMeasVec, tagMeasVec,
                    solverParameters.imuCalibration_,
                    aprilTagParameters.tagMeasCalibration_, gravity_rot,
                    aprilTagParameters.tagMeasCalibration_.projMat_, T_c_i);

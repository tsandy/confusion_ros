...
TagTrackerRos<StateType>::TagTrackerRos(ros::NodeHandle& node)
    : node_(node),
  ...
  std::string datafilePath = confusionPath_ + "/data";
  apriltag_module_ = std::unique_ptr<AprilTagModuleRos>(new AprilTagModuleRos(
      node, &conFusor_, datafilePath, configFile, TAG, &newTagMeasReady_));
  ...
  firstState->setTagReferenceFrameOffsets(
      apriltag_module_->getTagReferenceFrameOffsetsMapPointer());
  ...
  pubRtPose_ =
      node_.advertise<geometry_msgs::PoseStamped>("/imu_pose_prop", 10);
  imagePub_ = imageTransport_.advertise("/visualization_image", 1);

  // Listen for other user triggers
  subTriggerBatch_ =
      node_.subscribe("/trigger_batch", 1,
                      &TagTrackerRos<StateType>::triggerBatchCalCallback, this);
  subDrawDiagram_ =
      node_.subscribe("/draw_confusion_diagram", 1,
                      &TagTrackerRos<StateType>::drawDiagramCallback, this);

  // Start listening for measurements
  subImu_ = node_.subscribe(pt.get<std::string>("imu_topic"), 1000,
                            &TagTrackerRos<StateType>::imuCallback, this,
                            ros::TransportHints().tcpNoDelay());

  subImage_ =
      imageTransport_.subscribe(pt.get<std::string>("camera_topic"), 2,
                                &TagTrackerRos<StateType>::imageCallback, this);

  // Start the estimator on a separate thread
  estimatorThread_ = std::thread(&TagTrackerRos<StateType>::runEstimator, this);
  estimatorThread_.detach();

  std::cout << "[TagTrackerRos] Initialization complete." << std::endl;
}

template <typename StateType>
void TagTrackerRos<StateType>::imuCallback(
  ...
  Eigen::Vector3d w(imuMsg->angular_velocity.x, imuMsg->angular_velocity.y,
                    imuMsg->angular_velocity.z);

  conFusor_.addProcessMeasurement(std::make_shared<confusion::ImuMeas>(
      t_imu_source, a, w, &tagTrackerParameters_.imuCalibration_,
      &gravity_rot_));

  // Optionally publish the forward-propagated state at IMU rate
  if (forwardPropagateState_) {
    confusion::ImuMeas meas(t_imu_source, a, w,
                            &tagTrackerParameters_.imuCalibration_,
                            &gravity_rot_);
    imuPropagator_.addImuMeasurement(meas);

    if (tracking_) {
      confusion::ImuStateParameters state =
          imuPropagator_.propagate(t_imu_source);

      geometry_msgs::PoseStamped pose_msg =
          getMsg(state.T_w_i_, "/world", ros::Time(state.t()));
      ...
      tfBroadcaster_.sendTransform(tf::StampedTransform(
          getTfMsg(state.T_w_i_), ros::Time::now(), "world", "imu"));
    }
  }
...
template <typename StateType>
void TagTrackerRos<StateType>::imageCallback(
    const sensor_msgs::ImageConstPtr& msg) {
  if (tracking_) {
    confusion::ImuStateParameters state =
        imuPropagator_.propagate(msg->header.stamp.toSec());

    cv_bridge::CvImagePtr cv_ptr =
        cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::RGB8);
    apriltag_module_->projectTagsIntoImage(state.T_w_i_, "cam", cv_ptr->image);
...
template <typename StateType>
void TagTrackerRos<StateType>::runEstimator() {
  ...
  while (run_) {
    ...
    tracking_ = true;
    apriltag_module_->copyOutEstimateAfterOptimization();
...
template <typename StateType>
void TagTrackerRos<StateType>::stopTracking() {
  std::cout << "Stopping tracking" << std::endl;

  // Stop the estimator and wait for it to exit
  run_ = false;
  tracking_ = false;
  while (ros::ok() && !runEstimatorLoopDone_) ros::Rate(10).sleep();
...
template <typename StateType>
void TagTrackerRos<StateType>::startTracking() {
  if (run_) {
...
template <typename StateType>
void TagTrackerRos<StateType>::stopTracking(const std_msgs::Empty& msg) {
  stopTracking();
}

template <typename StateType>
void TagTrackerRos<StateType>::startTracking(const std_msgs::Empty& msg) {
  startTracking();
}

template <typename StateType>
void TagTrackerRos<StateType>::drawDiagramCallback(const std_msgs::Empty& msg) {
  std::cout << "User requested for a MHE diagram to be drawn." << std::endl;
  drawDiagramRequest_ = true;
}

template <typename StateType>
void TagTrackerRos<StateType>::triggerBatchCalCallback(
  ...
  auto& T_c_i_ptr = apriltag_module_->sensorFrameOffsets_["cam"];
...
template <typename StateType>
void TagTrackerRos<StateType>::publish(
    std::shared_ptr<confusion::State> statePtr,
  ...
  apriltag_module_->publishTagMarkers();
